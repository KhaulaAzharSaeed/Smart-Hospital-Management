...
# title
title = Label(top_frame, font = "evogria 45", bg = "#FFE2E2", width = 42, anchor = CENTER)
title.pack()
...
def about_window():
    ...
    # main content
    details_1 = (
        "Talking about the features of this Smart Hospital Management System, this project is aimed for a completely "
        "computerised management of our fictional hospital HOSPITALS. A patient can register themselves, view their details "
    ...
    about_title_lucifer = Label(right_frame, text = "", font = "evogria 16", anchor = CENTER, bg = "#FFE2E2")
    about_title_lucifer.grid(row = 12, column = 0, padx = 70, sticky = W)

    about_title_vats = Label(right_frame, text = "", font = "evogria 16", anchor = CENTER, bg = "#FFE2E2")
    about_title_vats.grid(row = 12, column = 0, padx = 5)

    about_title_meena_sir = Label(right_frame, text = "", font = "evogria 16", anchor = CENTER,
                                  bg = "#FFE2E2")
...
def insert_bydefault_data():
    operation = """INSERT INTO patient_details VALUES
					(01, "Udit Pati", 17, "M", 8375054875, "Delhi", "O+"),
					(02, "Robin Vats", 17, "M", 7567563156, "Delhi", "O+"),
					(03, "Rahul Roy", 18, "M", 8345671848, "Mumbai", "A+"),
					(04, "Aditya Manas", 16, "M", 7534586798, "Jaipur", "B+")"""
    cursor.execute(operation)
    # dbcon.commit()
...
if (by_default_data_confirm == 1):
    try:
        ...
        speak("Welcome to  Hospital. How can I help you?")
    except:
        speak("Data inserted in the Database")
        speak("Welcome to  Hospital. How can I help you?")
        pass
...
def appointment_submit():
    ...
    for row in cursor.fetchall():
        exis_value = row[2]
        #print(exis_value)
        #print("curr date is",date)
        if (str(exis_value) == str(date)):
            #print('not allowed')
            messagebox.showerror("Database", "This date is already taken")
            return
    ...
    details = """YOUR APPOINTMENT HAS BEEN APPROVED ON
...
STOP CORONA TOGETHER""".format(date)
